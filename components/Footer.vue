<template>
  <div class="footer p-5 text-light">
    <div class="container">
      <div class="row gy-4 justify-content-center text-center text-md-start">
        <!-- Logo + Download + Contact -->
        <div class="col-lg-3 col-md-6 col-12 logo">
          <img src="/Yarmook Logo (1).png" alt="Yarmook Logo" class="img-fluid mb-3" />
          <p class="text-light">Download our app</p>
          <div class="icons-link d-flex flex-wrap justify-content-start gap-2">
            <div class="google d-flex align-items-center gap-2 ps-3 pe-3 pt-2 pb-2 box-style">
              <img class="icon" src="/Google-logo.png" alt="Google" />
              <span class="text-capitalize span">Google Play</span>
            </div>
            <div class="apple d-flex align-items-center gap-2 ps-3 pe-3 pt-2 pb-2 box-style">
              <img class="icon" src="/Apple-logo.png" alt="Apple" />
              <span class="text-capitalize span">App Store</span>
            </div>
          </div>
          <div class="contact-us mt-4">
            <h6 class="text-capitalize">Contact us:</h6>
            <p class="mt-2">0123456789</p>
            <p class="work-time">Everyday 8 am - 12 am</p>
          </div>
        </div>

        <!-- Links -->
        <!-- <div class="col-lg-2 col-md-4 col-6">
          <h6 class="text-capitalize">Links</h6>
          <ul class="list-unstyled">
            <li><NuxtLink to="/">Home</NuxtLink></li>
            <li><NuxtLink to="/services">Services</NuxtLink></li>
            <li><NuxtLink to="/SpareParts">Spare Parts</NuxtLink></li>
            <li><NuxtLink to="/offers">Offers</NuxtLink></li>
            <li><NuxtLink to="/">Membership</NuxtLink></li>
          </ul>
        </div>

        
        <div class="col-lg-2 col-md-4 col-6">
          <h6 class="text-capitalize">Products</h6>
          <p>Product 1</p>
          <p>Product 2</p>
          <p>Product 3</p>
          <p>Product 4</p>
        </div>

    
        <div class="col-lg-2 col-md-4 col-6">
          <h6 class="text-capitalize">Help</h6>
          <p>FAQ</p>
          <p>Contact Us</p>
          <p>WhatsApp</p>
        </div>

        
        <div class="col-lg-2 col-md-4 col-6">
          <h6 class="text-capitalize">Follow Us</h6>
          <p>Facebook</p>
          <p>Instagram</p>
          <p>Twitter</p>
        </div> -->
      </div>
    </div>
  </div>
</template>

<style scoped>
.footer {
  background-color: #1e1e1e;
}
.logo img {
  max-width: 100px;
  height: auto;
}
.box-style {
  background-color: #040505;
  border-radius: 20px;
  border: 1px solid white;
  width: fit-content;
}
.icon {
  width: 15px;
}
.work-time {
  margin-top: -5px;
}
@media (max-width: 576px) {
  .span {
    font-size: 10px;
  }
  .box-style {
    width: 100%;
    justify-content: center;
  }
}
@media (max-width: 992px) {
  .icons-link {
    flex-direction: column;
  }
  .span {
    font-size: 13px;
  }
}
</style>
